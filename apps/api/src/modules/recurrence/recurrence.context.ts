...
/**
 * Per-request recurrence context (ADR-0025): resolves the authenticated caller to their
 * workspace (and, for record ownership, their user id) over the `DB` token. Every route scopes
 * to the id this resolves — never one from the client — so series are workspace-isolated by
 * construction (ADR-0015).
 */
@Injectable()
export class RecurrenceContext {
