...
/**
 * Emit the OpenAPI spec generated from the controllers/DTOs to a file, so CI can
 * publish it as an artifact and it can never silently drift from the routes
 * (issue #3, ADR-0025). A lazy db handle is passed so every DB-backed module
 * registers its controllers for introspection; postgres.js connects only on the
 * first query, and emitting never runs one — no database is required.
 */
