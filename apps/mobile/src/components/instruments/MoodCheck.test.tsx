...
describe('MoodCheck', () => {
  it('MoodCheck_initialRender_offersThreeMoodsAndSkip', () => {
    ...
    expect(labels).toEqual(['Good', 'Tense', 'Stressed', 'Skip'])
    expect(texts(r)).toContain('how was the block?')
  })

  it('MoodCheck_pickMood_showsQuietConfirmationAndReportsChange', () => {
    const onChange = vi.fn()
    const r = render(<MoodCheck onChange={onChange} />)
    const gut = r.root.findAllByType(Pressable).find(b => b.props.accessibilityLabel === 'Good')!
    act(() => {
      gut.props.onPress()
    })
    expect(onChange).toHaveBeenCalledWith('good')
    expect(texts(r)).toContain('Noted')
  ...
  it('MoodCheck_skip_dismissesViaOnDone', () => {
    ...
    const skip = r.root.findAllByType(Pressable).find(b => b.props.accessibilityLabel === 'Skip')!
    act(() => {
      skip.props.onPress()
    })
