...
| REQ-044 | Partial | `apps/mobile/src/onboarding/OnboardingGate.test.tsx`, `apps/mobile/src/onboarding/onboardingStore.test.ts`, `apps/mobile/src/api/tracking.test.ts` | First-run onboarding flow + one-boolean gate ship (ADR-0036). Onboarding-created **projects now persist** via the client `createProject` (`POST /api/tracking/projects`), fixing audit H8 — the create-project client + its request/parse are tested; the backend endpoint is integration-tested under REQ-001. The durable native "onboarded" flag (audit M11) is the one remaining piece — tracked by [#264](https://github.com/NexusHero/myDevTime/issues/264). |
