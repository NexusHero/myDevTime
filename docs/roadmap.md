...
UX with **Tackle's** AI automation & billing, plus an own AI layer on top
([ADR-0002](adr/0002-product-scope-unify-tyme-and-tackle.md)). Stack: Node.js/TypeScript backend
...
3. **LLM cost data** ([#17](https://github.com/NexusHero/myDevTime/issues/17)–[#20](https://github.com/NexusHero/myDevTime/issues/20)
   metering) — pricing ([#29](https://github.com/NexusHero/myDevTime/issues/29)) needs real
   per-feature cost numbers; instrument from the first LLM call, don't reconstruct later.
...
M3  #15 (calendar) ──► #16 (rules engine) ──► #17 (LLM proposals)
                                    ...
                                    │
M4  #21 (entitlements) ──► #22 (Stripe web)
                      └──► #23 (store IAP)
                                    │
...
Parallelism notes:
- #1 and #2 run in parallel from day one; #3–#6 are sequential on #2 (with #4∥#6 possible).
...
- Subscriptions purchasable on all three rails (#22, #23) with entitlements converging through
  #21; pricing decided and recorded as an ADR (#29).
...
- **Teams**: shared workspaces, roles, approval flows, team analytics (Tackle's team tier).
...
- **CSV import** from Tyme/Toggl/Tackle for switchers; English-beyond-de/en localization.
