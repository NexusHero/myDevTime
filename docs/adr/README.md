...
| [0043](0043-adopt-powersync-for-offline-sync.md) | **Adopt PowerSync for offline-first sync**: the client uses PowerSync's managed SQLite + upload queue + streaming pull + checkpoints instead of the hand-rolled `sync_outbox`/`sync_state`/`runSync`. Writes flow through `uploadData()` to our backend, which applies them via the **existing** `packages/domain` conflict engine — so interval conflicts are still **surfaced, never auto-merged** (REQ-006). Sync rules scope by `workspace_id` (isolation). Retains `packages/domain` + the ADR-0042 offline compute; retires the #188/#189 hand-rolled client sync; SDK behind the `LocalDb` port | Accepted (owner decision) — supersedes the client half of ADR-0019 & ADR-0040; bound by ADR-0005 |
...
| Drizzle ORM on the client (typed local SQLite queries via `sqlite-proxy` over the `LocalDb` port) | Adopt | ADR-0046 |
...
| `@tanstack/react-query` (server-state for non-PowerSync reads; `useQuery`/`useMutation`) | Adopt (online endpoints only — offline data stays on local SQLite/PowerSync) | ADR-0047 |
