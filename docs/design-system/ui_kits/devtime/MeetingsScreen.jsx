function MeetingsScreen() {
  ...
  const stateBadge = s =>
    s === 'insights'
      ? { tone: 'good', label: 'Insights ✓' }
      : s === 'transcript'
        ? { tone: 'neutral', label: 'Transcript' }
    ...
    <div style={{ padding: 28, maxWidth: 1120 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 18 }}>
        ...
        <div style={{ color: 'var(--ink-2)', fontSize: 'var(--fs-sm)' }}>
          Transkripte &amp; AI-Insights · nur mit Zustimmung
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
                ...
                return (
                  ...
                  >
                    <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      ></span>
                      <span
                        style={{ fontSize: 'var(--fs-sm)', fontWeight: 600, color: 'var(--ink)' }}
                      >
                        {m.title}
                      </span>
                    </span>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
          <Card
            ...
            action={sel.state !== 'upcoming' && <Badge tone="good">Transcript · de</Badge>}
          >
            {sel.state === 'insights' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                            ...
                            <Button
                              ...
                              onClick={() =>
                                setNote(
                                  'Creates a ' +
                                    target +
                                    ' item from a preview payload — after you confirm, never automatically.',
                                )
                              }
                            >
                              → {target}
                ...
                >
                  Every figure comes from the deterministic core — never the model. The transcript
                  is your data: export and deletion included.
                </div>
              </div>
            )}
            ...
            {sel.state === 'upcoming' && (
              ...
              >
                Starts at 15:00. Recording consent is granted (revocable per meeting) — the bot
                joins visibly and every participant sees the recording status.
              </span>
            )}
          </Card>
