      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
        {data.map(d => (
          <div key={d.n} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span
              style={{ width: 9, height: 9, borderRadius: 3, background: d.c, flexShrink: 0 }}
            ></span>
            <span
              style={{
                flex: 1,
...
function ReportsScreen() {
  ...
  const fmtWorked =
    range === 'year'
      ? Math.round(worked).toLocaleString('de-DE') + 'h'
      : Math.floor(worked) + ':' + String(Math.round((worked % 1) * 60)).padStart(2, '0') + 'h'
    ...
    <div style={{ padding: 28, maxWidth: 1120 }}>
      ...
      <div style={{ margin: '16px 0 0', maxWidth: 680 }}>
        <AIAskBar
          ...
          answers={{
            'Welches Projekt frisst mein Budget?':
              'Nordwind: 91% des 80h-Budgets verbraucht, Run-Rate 1,9h/Tag — erschöpft ~21.7. Alle anderen Projekte liegen unter 65%.',
            'Wie stehe ich beim Überstunden-Saldo?':
              '+9:30h Jahressaldo. Dein Median-Tag liegt bei 8:24h, 4 Minuten über Soll — der Saldo wächst also langsam weiter.',
          }}
        />
      </div>
            ...
            <div style={{ flex: '0 1 300px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 10, color: 'var(--ink-3)', marginTop: 4 }}>
                  Linie = passive Signale · <span style={{ color: 'var(--accent)' }}>▪</span> =
                  deine Check-ins
                </div>
              </div>
            </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 12, flexWrap: 'wrap' }}>
        ...
        <Card
          ...
          style={{ flex: '1.3 1 340px', minWidth: 0 }}
        >
        ...
        >
          <svg viewBox="0 0 300 110" style={{ width: '100%', display: 'block' }}>
            ...
            <polyline
              ...
              strokeLinecap="round"
              strokeLinejoin="round"
              style={{ transition: 'all var(--dur-slow) var(--ease-out)' }}
            />
            <polyline
            ...
            <text x="4" y="24" fontFamily="var(--font-mono)" fontSize="9" fill="var(--ink-3)">
              80h
            </text>
            <text x="130" y="108" fontFamily="var(--font-mono)" fontSize="9" fill="var(--live)">
              heute
            </text>
            <text x="248" y="94" fontFamily="var(--font-mono)" fontSize="9" fill="var(--ink-3)">
              21.7.
            </text>
          </svg>
          ...
          >
            ...
            <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--ink-2)', lineHeight: 1.5 }}>
              <b style={{ color: 'var(--ink)', fontFamily: 'var(--font-mono)' }}>{d.burn.neu}</b>{' '}
              <span style={{ color: 'var(--accent)' }}>■</span> neue Aufgaben ·{' '}
              <b style={{ color: 'var(--ink)', fontFamily: 'var(--font-mono)' }}>{d.burn.zu}</b>{' '}
              <span style={{ color: 'var(--good)' }}>■</span> geschlossen — {d.burn.fazit}
            </div>
          </div>
        </Card>
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 12, flexWrap: 'wrap' }}>
        ...
        >
          ...
          <div style={{ fontSize: 'var(--fs-2xs)', color: 'var(--ink-3)', marginTop: 4 }}>
            Die Box ist dein typischer Tag (25–75%), der Strich der Median, orange das Soll — liegt
            die Box rechts davon, baust du Überstunden auf.
          </div>
        </Card>
        {range === 'week' && (
          <Card
        ...
        {range === 'month' && (
          ...
          >
            ...
            {(() => {
                ...
                return (
                  ...
                  >
                    <span style={{ flex: 1, fontSize: 'var(--fs-2xs)', color: 'var(--ink-2)' }}>
                      <b style={{ color: 'var(--ink)' }}>{head}</b> — Buchungslücke: Arbeitstag ohne
                      Buchung.
                    </span>
              ...
              if (v === 0 || v === null)
                return (
                  <div style={{ marginTop: 12, fontSize: 'var(--fs-2xs)', color: 'var(--ink-3)' }}>
                    <b style={{ color: 'var(--ink-2)' }}>{head}</b> — keine Buchungen
                    (Wochenende/frei).
                  </div>
                )
              const entries =
                v === 'now'
                  ? [
                      ['Finanzo Review', 'var(--project-1)', '1:30'],
                      ['Nordwind Call', 'var(--project-3)', '0:45'],
                      ['Sync engine', 'var(--project-2)', '2:10'],
                    ]
                  : [
                      ['Finanzo API', 'var(--project-1)', v + ':10'],
                      ['Sync engine', 'var(--project-2)', v === 1 ? '0:50' : v - 1 + ':40'],
                    ]
              return (
                <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <span style={{ fontSize: 'var(--fs-2xs)', fontWeight: 700, color: 'var(--ink)' }}>
                    {head}
                    {v === 'now' ? ' · heute' : ''}
                  </span>
                  {entries.map(([n, c, h]) => (
