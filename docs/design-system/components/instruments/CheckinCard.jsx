...
export function CheckinCard({ onDone, compact = false }) {
  ...
  const submit = () => {
    setDone(true)
    if (onDone) onDone({ exhaustion: a1, detachment: a2 })
  }
  ...
  return (
    <div
      style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: compact ? '2px 0' : 0 }}
    >
      {QUESTIONS.map(([q, val, set, [lo, hi]]) => (
        <div key={q} style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 10, color: 'var(--ink-3)', width: 52, textAlign: 'right' }}>
              {hi}
            </span>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span style={{ fontSize: 10, color: 'var(--ink-3)' }}>
          10 Sekunden · bleibt auf deinem Gerät
        </span>
      </div>
    </div>
  )
