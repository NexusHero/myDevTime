...
export function LoadMeter({ score = 42, label, width = 300 }) {
  ...
  const zoneColor =
    zone === 'ok' ? 'var(--good)' : zone === 'elevated' ? 'var(--warn)' : 'var(--bad)'
  const zoneLabel =
    label || (zone === 'ok' ? 'Im grünen Bereich' : zone === 'elevated' ? 'Erhöht' : 'Kritisch')
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      >
        ...
        <circle
          ...
          stroke={zoneColor}
          strokeWidth="3"
          style={{ transition: 'cx var(--dur-slow) var(--ease-out)' }}
        />
      </svg>
