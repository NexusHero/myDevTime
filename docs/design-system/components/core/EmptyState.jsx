...
export function EmptyState({ icon = 'plus', title, hint, action, compact = false }) {
  return (
    <div style={{
      ...
      gap: 10, padding: compact ? '22px 18px' : '38px 24px',
      border: '1.5px dashed var(--border-strong)', borderRadius: 'var(--radius-xl)',
    }}>
      <span style={{
      ...
      <div style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 'var(--fs-md)', color: 'var(--ink)' }}>{title}</div>
      {hint && <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--ink-2)', maxWidth: 380, lineHeight: 'var(--lh-normal)' }}>{hint}</div>}
