...
| REQ-011 | Deterministic rules engine: ordered versioned matchers → categorization actions, dry-run preview, `rule:<id>@<version>` provenance | ADR-0005, [#16](https://github.com/NexusHero/myDevTime/issues/16) | Partial — the pure `rules/engine` core ships: a `Rule` is an ordered, versioned matcher → action; `evaluate` returns the **first** matching rule's action (order is intent) with `rule:<id>@<version>` provenance (`ruleProvenance`), `matches` is an AND over the present conditions (note-contains case-insensitive, source, project-empty, start-within half-open, weekday-in), `orderedRules` sorts by order then id and drops disabled, and `dryRun` previews the whole set over many subjects **without applying anything** (ADR-0005). **Deferred:** rule persistence + the apply/dry-run API endpoints + the client rule editor — with an acceptance-tier test |
...
| REQ-014 | AI summaries & standup reports: narrative around domain-computed numbers (slot integrity), read-only artifacts, plain-template degradation | ADR-0005, [#19](https://github.com/NexusHero/myDevTime/issues/19) | Partial — the AI **day-briefing** ships (credit-priced, idempotent, degrades to the factual summary). The **standup** core now ships too: `reporting/standup.buildStandup` structures grouped tracked time into a report with **protected numeric slots** (`standupSlots` = every line duration + both totals), `renderStandupPlain` is the AI-free degradation (its numbers *are* the slots), and `slotsPreserved` verifies an AI draft kept every number — the LLM narrates around the figures but can never change one (slot integrity, ADR-0005). **Deferred:** the standup endpoint + client surface, with an acceptance-tier test |
