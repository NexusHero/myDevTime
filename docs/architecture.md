...
| REQ-028 | Attendance: clock-in/out, breaks, effective-dated target-hour schedules, overtime balance, project-coverage reconciliation, configurable break-rule check (ArbZG §4 preset) | ADR-0010, [#36](https://github.com/NexusHero/myDevTime/issues/36) | In progress — deterministic work-day core landed in `packages/domain/attendance` (punch-pair net time + overtime balance vs a weekly target); effective-dated schedules, break-rule (ArbZG §4) check, project-coverage reconciliation, persistence + endpoint still to come |
