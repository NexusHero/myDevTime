...
| REQ-074 | Calm canvas + Sevi first run: the week defaults to the accepted plan + now-line only — reality, ghosts, life shades and capacity trace live behind layer chips, never all loud at once; blocks are redrawn (type hierarchy, project colour as edge, states planned/live/done/missed); empty edge hours compress. An empty planner is Sevi's stage: 2–3 questions produce the first ghost week via the plan-apply seam (no demo data). The redesign lands only after a before/after artifact is approved | ADR-0072/0071 | Partial — the package ships and is fully tested (pure `compressWindow` ≥90 % with the boundary cases; the four block states AA-held across light+dark × three accents; RN + web FullCalendar restyled to colour-as-edge; per-user layer chips on the preferences contract; Sevi's deterministic first-run ghost week through the seam, provenance `planner-firstrun`). The before/after artifact (`docs/design/artifacts/planner-calm-canvas-before-after.html`) awaits owner approval — the gate — before the row moves to Done |
