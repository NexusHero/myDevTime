...
| REQ-006 | Cross-device sync: offline-first, idempotent, resumable; conflicts never silently merge into wrong durations — server-authoritative delta sync with a deterministic per-entity conflict policy | ADR-0019, [#9](https://github.com/NexusHero/myDevTime/issues/9) | Proposed — protocol decided (ADR-0019); engine is #9 Phase 2 |
