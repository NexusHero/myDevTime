...
| REQ-010 | Calendar integration (Google/Microsoft, read-only): encrypted revocable grants, events normalized into candidate entries, never auto-committed | [#15](https://github.com/NexusHero/myDevTime/issues/15) | Proposed; go-live handback documented in `docs/integrations/calendar-oauth-handback.md` (exact external steps + remaining adapter file); the **live Google OAuth flow** now ships: `GET :id/authorize` (signed-state authorize URL for the consented scopes), `GET :id/callback` (verifies state, exchanges the code at Google's token endpoint, seals tokens into the AEAD vault), `freshAccessToken` (refresh-on-expiry, preserving the refresh token), the `GoogleCalendar` adapter (`calendars/primary/events` window → neutral `ExternalEvent`, vendor JSON confined to one file), and `GET google-calendar/preview` returning `planImport` **proposals** — consent-gated, writes nothing (ADR-0005). All env-gated (`CONNECTOR_GOOGLE_CALENDAR_CLIENT_ID/SECRET`, `CONNECTOR_MASTER_KEY`, redirect base); unconfigured stays honestly 'not configured'. **Microsoft/Outlook now ships the same vertical:** `microsoft-calendar` is registered (`Calendars.Read`), the Graph token/authorize endpoints are wired, and the `MicrosoftCalendar` adapter maps `/me/calendarView` → the neutral event (vendor JSON in one file); the preview route is generalized to `:id/preview` (google path kept, additive) so any OAuth calendar connector previews through it. **Apple Calendar is made honest:** it has no OAuth calendar API, so `apple-calendar` is registered as `auth:'native'` (EventKit) with an injectable native seam that is honestly `unavailable` on server/web/CI (never fabricates events) — the OAuth connect is no longer offered for it. **Client UI now ships:** the Profile Integrations section has a real **Connect** affordance (opens the authorize URL; surfaces the honest 409 "not configured" instead of a fake connect) and a **calendar import preview** rendering `planImport` proposals as ghost rows ("nothing is booked"); the client preview call is connector-generic. **External handback:** real Google + Azure app client id/secret + consent-screen verification; a native EAS build to bridge EventKit for the Apple live read |
...
| REQ-045 | Reports/analytics export: export the Reports dashboard view (summary tiles, breakdowns, budget burn-down, heatmap) as CSV/PDF from the deterministic Reports data — distinct from the timesheet/invoice export (REQ-009) | [#265](https://github.com/NexusHero/myDevTime/issues/265) | Done — the deterministic **`reportToCsv`** core (`reporting/export`) turns the Reports view-model (window · total/billable hours · billable money · overtime · per-project tracked time · budgets) into a labelled, RFC 4180-escaped **CSV** — every figure formatted in the core (hours/money 2dp, utilization integer %), byte-identical per input, empty report → headers only (ADR-0005). The **PDF rendition** now ships too: `POST /api/tracking/reports/export?format=pdf` (behind `AuthGuard`, bounded DTO) renders the same view-model server-side through the REQ-009 PDF infra; the client `fetchReportsPdf` posts the live `ReportsData` and downloads `mydevtime-reports-<window>.pdf`. The Reports **Export** control is live for both formats: on web it downloads the CSV/PDF; on native the download is unavailable and the UI says so (never a silent no-op) |
...
| REQ-064 | Family-market layer filter + stroke-shield icon (design v17 §F6.5 + icon rule): the Planner header carries a **Work / Life / Both** filter (default **Both**) — one person, one timeline, the filter only changes what is *shown*, never what exists (capacity and price still read the full block set). The protection flag and every future 🛡 surface render a **stroke-shield SVG** (`currentColor`, 24px grid, 2px stroke), **never** the coloured 🛡 emoji, which would break the monochrome icon language. Deterministic filter (ADR-0005) | ADR-0067/0005 | Partial — the pure `planner/layer` filter (`inLayer`, `PLANNER_LAYERS`, default Both first) ships and the **Planner header** wires the Work/Life/Both pills to it (shown blocks filtered, capacity/price untouched), and the drawer's protection row now renders the `shield` Icon glyph instead of the 🛡 emoji. **§F6 person-colour shades ship:** deterministic per-person **sage** shades of the `life` tone (`design/people.personShade`/`personShadeIndex` over `lifeShades`, a shared FNV-1a `hash`), slot 0 = the plain `life` token, no new rainbow palette — a person is not a project. **The one-link partner-light share ships:** the pure `sharing/freebusy` core (`toFreeBusy`/`freeGaps`) projects an owner's blocks to a **Free/Busy-only** `FreeBusySlot` whose type carries no title/project/note, so leaking detail is unrepresentable (a 🛡 block reads as plain busy); the `sharing` module persists workspace-scoped one-link grants (`partner_shares`, migration 0020) and the **public** `GET /api/sharing/:token/freebusy` serves only busy spans + free gaps (no auth — the link is the credential — never a detail column), with a negative-isolation integration test that a private note never crosses the boundary and that the view is workspace-scoped and revocable. **Google/Apple calendar sync ships as a port (ports-and-adapters, skill §2.2):** the pure `calendarsync/merge` core (`mergeCalendar`) diffs a provider's events against already-imported blocks into **new / changed / orphaned proposals** keyed on the event uid — ghost blocks to confirm, never auto-booked; the narrow `CalendarPort` confines any vendor SDK to a single adapter (`google`/`apple`), with a `NullCalendar` graceful-degradation default and a `planImport` that gates on **consent** (REQ-025) then availability before merging (a down provider degrades to an empty plan, never throws); `apple-calendar` is registered in the connector registry (least-privilege scopes). The **live Google adapter now ships** (see REQ-010), and a **Microsoft Graph adapter + an Apple EventKit native seam** now sit behind the same `CalendarPort` (`microsoft`/`apple` providers). **The partner-light Free/Busy share now has a client UI:** the Settings "Free / Busy sharing" section lists the workspace's share links (label · created · "Busy times only" badge · the copyable public URL), creates a new link (`POST /api/sharing`), and revokes one behind a confirm (`DELETE /api/sharing/:id`) — the copy states plainly that a holder sees only *when* you're busy, never a title/project/note. **The KI5 photo/mail import ships as a port:** the pure `photoimport/schedule` core (`toSeriesProposals`) validates AI-extracted lessons and shapes them into weekly `RecurrenceRule` **series proposals** carrying `ai-proposal` provenance and `confirmed: false` — the AI extracts, the human confirms, **nothing is auto-booked** (ADR-0005); the narrow `ScheduleImportPort` confines any vision SDK to one adapter, with a `NullScheduleImport` default and a `planPhotoImport` that gates on **Pro** (violet, one credit — charged only on a confirmed booking), then **consent** (REQ-025), then availability. The **live vision adapter stays spike-gated** behind this port. **Deferred (acceptance criteria in ADR-0067):** the live-provider adapters for calendar sync + photo import (integration spike) and the client ghost-confirm UI — each with an acceptance-tier test |
| REQ-065 | Evening Companion / wellbeing (grounded day-in-review): a deterministic day-load + wellbeing signal (calendar load, overtime, break shortfall, meeting density, mood, plan drift → a banded light/normal/heavy/overload level) with a longitudinal personal baseline + pattern flags, narrated once each evening by the grounded LLM into **one** warm summary + **one** gentle forward suggestion (proposal-only, credit-metered, degrades to a deterministic template) — weaving the existing MoodCheck / Feierabend / Balance / plan-adherence signals into a single caring voice that never books | ADR-0069/0005/0011 | Done — the pure `wellbeing` core ships exhaustively tested (≥90 %, ADR-0005): `reviewDay` → a banded load level + structured `WellbeingSignal`s (long-day, overtime, break-shortfall, back-to-back-meetings, meeting-heavy, plan-overrun, low-mood), and `computeBaseline` → a personal normal band, trend, and `consecutive-heavy-days` / `weekday-overbook` pattern flags (short history → wide band, no false flags). `POST /api/ai/evening-companion` (behind `AuthGuard`) runs the core (free) then a grounded narration — one credit **only** on a real `ai-proposal`, degrading to a caring deterministic template when the provider is down / no credits / an absence day; the suggestion's `kind` is always deterministic, only its phrasing is the model's, provenance always `ai-proposal`. The endpoint now **persists** the day's deterministic `loadScore` to the workspace-scoped `wellbeing_days` table (idempotent upsert per (workspace, user, day), mood deliberately not stored) and calibrates the baseline over the real 90-day series — no client-supplied history. It reads the **real worktime feed** for the overtime + break-shortfall signals (overtime = worktime-summary balance, break shortfall = summed shift shortfall), falling back to request values only when no completed shift exists. The Today `EveningCompanionCard` gathers the day's real signals, renders the band + week-trend + top signals + the warm message with honest provenance, and offers the one suggestion as a proposal confirmed in the Planner (nothing auto-booked). **Deferred:** a consented mood store (mood signal stays honestly omitted until it lands) |
| REQ-066 | Ticket/issue import (GitHub Issues + Azure DevOps Work Items): read the caller's assigned issues through one env-gated `IssueImportPort`, map them to **candidate task proposals** (deterministic, deduped), preview them (writes nothing), and create tasks only on explicit user confirm — import proposes, the human books (ADR-0005) | ADR-0070/0005/0033 | Done — the pure `issueimport` core ships (`toTaskProposals`: `ExternalIssue` → `CandidateTaskProposal`, dedup vs already-imported + in-batch, closed-filter, deterministic order, 100 % lines). Behind the narrow `IssueImportPort` (ports-and-adapters) a **GitHub** adapter (`GET /issues?filter=assigned`, vendor JSON confined to one file) and an **Azure DevOps** adapter (two-step WIQL → work-item batch) map to the neutral `ExternalIssue`; `azure-devops` is registered in the connector registry (category issues, oauth2, `vso.work`) with its authorize/token endpoints (GitHub already registered). `GET /api/connectors/:id/issues/preview` (AuthGuard, the same consent + sealed-token gate as the calendar preview) now resolves the caller's **already-imported keys** from the workspace-scoped `imported_issues` store and feeds them into the deterministic dedup, so a re-preview never re-proposes a ticket already imported; it returns `{ proposals, status }` and still **writes nothing** — an unconfigured/unconsented deployment returns an honest 409 / `unavailable`, never a fake import. A companion `POST /api/connectors/:id/issues/import` (AuthGuard + consent gate) records the (connector, externalKey → taskId) links only — idempotent per (workspace, user, connector, externalKey), never creating a task itself. The client `IssueImportCard` (ProjectsScreen) previews the proposals, lets the user select, creates a task per selection via the existing `POST /api/tracking/tasks`, then records the imported links — proposal-only. **Deferred:** the live Azure DevOps OAuth grant shape (`response_type=Assertion`). **External handback:** a real GitHub OAuth app + Azure DevOps org/project + credentials |
| REQ-067 | Sevi live-load trigger core (buddy P1): a pure `wellbeing/liveLoad` that folds the **intraday** worktime feed (hours worked today · continuous focus since the last break · back-to-back-meeting streak · overtime accrued today) into a banded escalation `calm / watch / speak-up` plus typed reasons. The `speak-up` boundary is **relative to the person's own baseline** (`computeBaseline`, H3 — never a fixed number) **plus a few universal hard caps** (ArbZG >6 h worked without a break, approaching 10 h, several heavy days in a row). *When* Sevi speaks is 100 % this deterministic core — the LLM is never in that path (ADR-0005) | ADR-0071/0005/0066 | Verified — the pure `wellbeing/liveLoad` core ships exhaustively table-tested (every ArbZG cap boundary, watch band, own-baseline exceedance, +Infinity thin-history guard); `GET /api/wellbeing/load-history` feeds the client-side baseline; `useLiveLoad` derives the intraday input from the real worktime feed (net punch-clock day attribution; absent signals honestly zero) and the Today `SeviWatch` renders the calm `role="status"` voice only on a deterministic `speak-up` — browser-verified (hard day speaks, calm day is silent, opt-out and 🛡 suppress) |
| REQ-068 | Sevi consented mood store (buddy P3): the punch-out **MoodCheck** value is finally **persisted — but only under explicit opt-in consent** (REQ-025 pattern), server-side (so the grounded companion can weave mood over weeks and across devices), **never shared, exported, or paywalled**, and **erasable in one action** (delete the whole mood history). Extends the workspace-scoped `wellbeing_days` store (idempotent upsert per (workspace, user, day)); with consent off, mood stays honestly absent — no silent capture. Sevi states bands/signals, **never** a diagnosis (no "burnout score") | ADR-0071/0005/0066 | Verified — the `wellbeing_moods` store ships (migration 0026): consent enforced server-side (honest 409, nothing stored), idempotent per-day upsert, newest-first history, one-action full delete, negative workspace isolation; MoodCheck persists via `onChange` only under consent; the companion weaves the stored mood into the `low-mood` signal only under consent; `moodPatterns` flags a weekday only at ≥3 samples with median ≤2 (Reports note), and the `MoodEaseCard` offers one confirmable lighter-day shrink — browser-verified lifecycle incl. delete-all and isolation |
...
| REQ-075 | Meeting detail (issue [#375](https://github.com/NexusHero/myDevTime/issues/375)): a meeting entry carries **where it is, who is in it, and how to join it** — a location, an attendee list with each person's reply, and a conference link with its provider — so the entry detail answers what actually distinguishes a meeting from any other block. Third-party personal data: never exposed by the Free/Busy share path (REQ-064), and absent values render as absent, never as a placeholder | ADR-0076/0063/0005, REQ-025/060/064 | Partial — the vertical ships schema → API → client → UI: `recurring_entries` gains `location` / `attendees` (jsonb) / `conference_url` / `conference_provider` (migration 0028), `seriesToOccurrences` carries all four verbatim onto every occurrence (ADR-0005), the create DTO validates them (bounded sizes, email/URL shapes, only `name` required per attendee — a connector that supplies no address must not force an invented one), the client occurrence/series parsers surface them, and the entry drawer renders a location row, an attendee list stating each reply **in words** (never colour alone, REQ-043), and a join action — each rendered only when the entry actually carries it. Free/Busy stays isolated by construction: `freeBusyForShare` selects only `startedAt`/`endedAt` and its response type has no field these could occupy. **Deferred:** connector mapping (Google/Graph both supply all three), which needs the not-yet-existing "import a synced meeting as a series" path — calendar sync today yields `ExternalEvent` ghost-block proposals and never writes a series; and **in-app editing now ships**: `PATCH /api/recurrence/:id` patches a series' authored detail (title / location / conference link + provider / note) — a field left out is untouched, a field set to `null` is **cleared**, so a wrong location can be taken back rather than only overwritten; the drawer offers the editor exactly where a series exists (an occurrence-derived entry) and never for a canvas block, which is local state with nothing behind it. **Attendees stay non-patchable on purpose:** they are third-party people mirrored from a calendar, and with no invite mechanism an app-authored guest list would store names nobody is ever told about |
...
## Sevi speaks only when the deterministic core says so (REQ-067/069, ADR-0071/0005)

Whether Sevi's real-time watch may speak is decided entirely by two pure cores: `evaluateLiveLoad`
folds the intraday worktime feed against the person's **own** baseline plus the universal ArbZG
caps, and `decideNudge` gates delivery on opt-in, quiet hours, an active 🛡 protected block, and
the shared daily cap. The LLM sits **only** behind the optional phrasing step — it can never cause
a voice the cores did not already authorize, and delivery waits for the protected-windows answer so
a ping can never race into an active 🛡 block.

![Sevi speaks only when the deterministic core says so (REQ-067/069, ADR-0071/0005) — diagram](diagrams/architecture-11.svg)
...
```mermaid
sequenceDiagram
    participant W as useWorktime / useLoadHistory (client)
    participant LL as evaluateLiveLoad (domain, pure)
    participant NP as decideNudge (domain, pure)
    participant P as NotificationPort (Null / web / native)
    participant U as Today (SeviWatch, role=status)
    W->>LL: intraday input + own baseline (normalHigh, +Infinity on thin history)
    LL-->>NP: LiveLoad {level, reasons, hardCapHit}
    NP->>NP: opt-in? quiet hours? 🛡 now (awaited, never raced)? shared daily cap?
    alt deliver
        NP-->>U: calm voice (deterministic text — the LLM may only rephrase, never trigger)
        NP->>P: notify(title, body) — degrades to in-app when the vendor is absent
    else suppressed
        NP-->>U: silence (🛡/quiet ⇒ one later digest, REQ-057)
    end
```

</details>

## A Sevi suggestion becomes a plan change only on explicit confirm (REQ-070, ADR-0071/0005)

Every Sevi proposal — the Scrum-Master's relief candidate, the Companion's protect-morning, the
lighter-day ease, life care's protect-an-evening — funnels through the **one** plan-apply seam.
The endpoint mutates nothing until the user's explicit confirm call arrives; a mutation writes a
NEW accepted plan version (the prior version stays untouched) or an idempotent protected-time row,
and returns the applied change. Nothing is ever auto-booked.

![A Sevi suggestion becomes a plan change only on explicit confirm (REQ-070, ADR-0071/0005) — diagram](diagrams/architecture-12.svg)
...
```mermaid
sequenceDiagram
    participant V as Sevi voice (advisory / companion / ease / life care)
    participant U as User
    participant API as planner module (POST /api/planner/apply)
    participant DB as Postgres
    V-->>U: one proposal (provenance ai-proposal, figures from the pure cores)
    U->>API: confirm {proposal} — the ONLY mutating call
    API->>API: AuthGuard → workspace scope (foreign plan ⇒ 404)
    alt protect-time
        API->>DB: INSERT protected_times (exact duplicate ⇒ idempotent no-op)
    else move/shrink-block
