...
| REQ-004 | Timers & manual entries: one running timer (DB-enforced, per-workspace), reboot-safe (running = persisted start instant, clock derived), manual create/edit/split/delete validated by the tracking core, provenance `source` on every entry; offline-first local store is the PowerSync client (ADR-0043) | [#8](https://github.com/NexusHero/myDevTime/issues/8) | API done (#8); client offline store = PowerSync (ADR-0043); numbers still computed by `packages/domain` over the local rows (ADR-0042), so offline == online |
...
| REQ-006 | Cross-device sync: offline-first, idempotent, resumable; conflicts never silently merge into wrong durations — server-authoritative delta sync (per-entity versioning via a DB trigger, tombstones for deletes) with a deterministic per-entity conflict policy in `packages/domain`. The client is **PowerSync** (managed local SQLite + upload queue, self-hosted); it uploads its intercepted CRUD queue to `/sync/upload`, where `resolveCrudWrite` applies the same policy (interval conflicts surfaced, never auto-merged). `/sync/push`+`/sync/pull` remain the deterministic engine the upload endpoint reuses | ADR-0019, ADR-0043, [#9](https://github.com/NexusHero/myDevTime/issues/9) | Server engine + `/sync/upload` done (#9; [#195](https://github.com/NexusHero/myDevTime/pull/195)); client sync engine = PowerSync self-hosted (ADR-0043); SDK adapter + retiring the hand-rolled client store tracked in [#193](https://github.com/NexusHero/myDevTime/issues/193) |
...
apps/api  (NestJS on the Fastify adapter — module-per-domain, ADR-0025)
  ...
  ├─ /api/sync        sync             offline-first cross-device sync (REQ-006); /sync/upload = PowerSync CRUD write target (ADR-0043)
