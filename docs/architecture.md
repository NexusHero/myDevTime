...
| REQ-017 | Stripe subscriptions on web: Checkout, Billing portal, signature-verified idempotent webhooks, Stripe Tax | ADR-0006, [#22](https://github.com/NexusHero/myDevTime/issues/22) | In progress (#22) — signature-verified webhook **adapter** done (`PaymentProviderPort` → `EntitlementEvent`, Stripe SDK confined to `billing/payments/stripe`, Phase A); Checkout/Billing-portal routes + webhook endpoint + customer↔workspace link pending (Phase B) |
